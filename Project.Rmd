...
required_packages <- c("tidyverse", "glmnet", "tidymodels", "caret", "car", "xgboost", "Metrics", "MASS", "pdp", "usmap", "scales", "gganimate", "gifski", "patchwork", "randomForest",
                       "kernlab", "caretEnsemble")
...
wide_cs <- charging_stations %>%
  ...
  rename(charging_sttns_fast = `Publicly available fast`,
...
combined_data = combined_data %>% 
  rename(Coal_Share = `Coal, Peat and Manufactured Gases (Net_Produce)`,
...
test_data <- test_data %>%
  dplyr::select(-c(BEV_Sales, BEV_Stock, PHEV_Stock, PHEV_Sales, FCEV_Stock, FCEV_Sales)) %>%
  dplyr::mutate_at(vars(-c("country", "year", "Total_Sales", "Total_Stocks", "region")), scale) 
```
...
- Therefore, we will try to use an ensemble learning algorithm next to see if we can improve on our scores. Perhaps, the underlying relationship isn't all that monotonous, which is not captured by this model.
...
for (i in 1:NMC){
  ...
  gbm_fit1 <- train(formula_model, data = train_data, method = "rf", trControl = fit_control, verbose = FALSE, verbosity = 0,
                    tuneLength = 5)
  
  # 1. Generate predictions on the test data
  predictions <- predict(gbm_fit1, newdata = test_data)
  
  # 2. Extract the actual Total_Sales values
  actuals <- log(test_data$Total_Sales)
  
  # 3. Calculate performance metrics
  
  # RMSE
  rmse_all[i] <- sqrt(mean((predictions - actuals)^2))
  
  # MAE
  mae_all[i] <- mean(abs(predictions - actuals))
  
  # R-squared
  rss <- sum((predictions - actuals)^2)  # Residual sum of squares
  tss <- sum((actuals - mean(actuals))^2)  # Total sum of squares
  R_squared_all[i] <- 1 - (rss / tss)
}


max(R_squared_all[R_squared_all > 0])
...
# best_model_params <- gbm_fit1$bestTune
# model_results <- gbm_fit1$results %>% 
#   filter(eta == best_model_params$eta,
...
#          nrounds == best_model_params$nrounds,
...
gbm_fit1 <- train(formula_model, data = train_data, method = "svmRadial", trControl = fit_control, verbose = FALSE, verbosity = 0,
                    tuneLength = 5)
...
rbind(getTrainPerf(all_in_one$"svmRadial"),
      getTrainPerf(all_in_one$"rf"),
      getTrainPerf(all_in_one$"xgbTree"))
...
t = test_data %>% 
  dplyr::select(c("Total_Sales", "region", "Nuclear_Share", "percent_renewable_electricity", "ratio_renew_nonrenew",
                  "annual_tavg", "charge_sttns"))

# 1. Generate predictions on the test data
predictions <- predict(all_models_ensemble, t)
...
```{r}
# Assuming the model was trained on the original Total_Sales values
...
# 1. Generate predictions on the test data
predictions <- predict(gbm_fit1, newdata = validation_data)

# 2. Extract the actual Total_Sales values
actuals <- log(validation_data$Total_Sales)
