...
isl = combined_data %>%
  ...
  mutate(log_pub_slow =  log(`Publicly available slow`)) %>% 
  mutate(log_pub_slow = approx(x = year, y = log_pub_slow, xout = year, rule = 2)$y) %>% 
  mutate(`Publicly available slow` = round(exp(log_pub_slow))) %>% 
...
jpn = combined_data %>% 
  ...
  mutate(
    rate_change_fast = 1 + (`Publicly available fast` - lag(`Publicly available fast`)) / lag(`Publicly available fast`),
    rate_change_slow = 1 + (`Publicly available slow` - lag(`Publicly available slow`)) / lag(`Publicly available slow`)
  )


# Fill in the missing values in `publicly_available_slow` using rate_change_fast
for (i in sum(is.na(jpn$`Publicly available slow`)):1) {
  jpn$`Publicly available slow`[i] = jpn$`Publicly available slow`[i + 1] / jpn$rate_change_fast[i + 1]
}
  
jpn = jpn %>% 
  dplyr::select(-c(rate_change_fast, rate_change_slow)) %>% 
  mutate(`Publicly available slow` = round(`Publicly available slow`))
...
loss_cons = combined_data %>%
  ...
  mutate(Electricity_Distrib_Loss = case_when(
    country == "Brazil" ~ 0.155 * `Electricity (Net_Produce)`,
    country == "China" ~ 0.05 * `Electricity (Net_Produce)`,
    country == "India" ~ 0.2 * `Electricity (Net_Produce)`
  )) 
combined_data %>% 
...
- With that out of the way, we can proceed with the actual machine learning analysis, where we split the data into train/test sets, perform LASSO, and the regression.
...
combined_data = combined_data %>% 
  mutate(percent_renewable_electricity = `Total Renewables (Hydro, Geo, Solar, Wind, Other) (Net_Produce)` / `Electricity (Net_Produce)`) %>% 
  mutate(non_renewable = `Electricity (Net_Produce)` - `Total Renewables (Hydro, Geo, Solar, Wind, Other) (Net_Produce)`) %>%
  mutate(ratio_renew_nonrenew = `Total Renewables (Hydro, Geo, Solar, Wind, Other) (Net_Produce)` / non_renewable) %>%  
  dplyr::select(-non_renewable)
